...
const props = defineProps<{
  conditions: Array<{ weather: string | null; time: string | null }>
  type: string
  bgClass: string
}>()

const emit = defineEmits<{
  (e: 'changeConditions', type: string, conditions: any[]): void
  (e: 'deleteCondition', type: string, index: number): void
}>()

const changeCondition = (evt: Event, key: string, index: number) => {
  const newConditions = Array.from(props.conditions).map(c => ({ ...c }))
  newConditions[index] = { ...newConditions[index], [key]: (evt.target as HTMLSelectElement).value }
  emit('changeConditions', props.type, newConditions)
}
</script>
